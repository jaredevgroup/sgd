const menuPage = {
    ...
    initEvents: function () {
        let options = { ...menuPage.options };
        let events = { ...menuPage.events };
    ...
    options: {
        ...
        dataTable: {
            ...
            order: [[1, 'desc']],
            ajax: {
                url: `${urlBaseWebApi}/api/menusistema/buscar-menusistema`,
            ...
            columns: [
                ...
                { data: 'Url', title: 'Url', orderable: false, render: (data) => `<a href="${(data.replace(data, urlBase))}">${data}</a>` },
    ...
    events: {
        ...
        tblResultadosOnPreXhr: function (e) {
            if ($(e.currentTarget.id == menuPage.options.idTblResultados)) $(menuPage.options.idBtnBuscar).prop('disabled', true);
            funciones.loading(`#${e.currentTarget.parentElement.parentElement.parentElement.id}`).Open();
        },
        tblResultadosOnXhr: function (e) {
            if ($(e.currentTarget.id == menuPage.options.idTblResultados)) $(menuPage.options.idBtnBuscar).prop('disabled', false);
            funciones.loading(`#${e.currentTarget.parentElement.parentElement.parentElement.id}`).Close();
        },
        btnBuscarClick: function (e) {
            menuPage.methods.buscar();
        },
        btnDetailsClick: function (e) {
            ...
            const options = { ...menuPage.options };
                ...
                <tr>
                    <td colspan="${cantidadColumnas}">
                        <div class="card card-secondary">
                            ...
                            <div class="card-body">
                                <a href="${urlBase}menu/nuevo/${menuId}" class="btn btn-primary">Nuevo</a>
    ...
    methods: {
        buscar: function () {
            $(menuPage.options.idTblResultados).DataTable().ajax.reload();
        },
        confirmarCambiarFlagActivo: function (id, flagActivo) {
            let title = "Confirmar";
            let text = `¿Desea ${(flagActivo ? "inactivar" : "activar")} el registro?`;
            menuPage.options.cambiarFlagActivoSwal.title = title;
            menuPage.options.cambiarFlagActivoSwal.text = text;
            Swal.fire(menuPage.options.cambiarFlagActivoSwal)
                .then(menuPage.methods.cambiarFlagActivo.bind({ id, flagActivo }));
        },
        cambiarFlagActivo: function (result) {
            if (!result.isConfirmed) return;
            let url = `${urlBaseWebApi}/api/menusistema/cambiar-flagactivo-menusistema/${this.id}/${this.flagActivo}`;
            let init = {};
            init.method = 'PUT';
            init.credentials = 'include';
            fetch(url, init)
                .then(funciones.managedResponseFetchToJson)
                .then(menuPage.methods.finalizarCambiarFlagActivo);
        },
        finalizarCambiarFlagActivo: function (data) {
            ...
            menuPage.options.finalizarCambiarFlagActivoSwal.title = title;
            menuPage.options.finalizarCambiarFlagActivoSwal.icon = icon;
            $(document).Toasts('create', menuPage.options.finalizarCambiarFlagActivoSwal);
            $(menuPage.options.idBtnBuscar).trigger('click');
        },
        cambiarOrden: function (data) {
            let url = `${urlBaseWebApi}/api/menusistema/cambiar-orden-menusistema/${data.id}/${data.orden}`;
            let init = {};
            init.method = 'PUT';
            init.credentials = 'include';
            fetch(url, init)
                .then(funciones.managedResponseFetchToJson)
                .then(menuPage.methods.finalizarCambiarOrden);
        },
